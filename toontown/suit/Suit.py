...
bw=(('finger-wag', 'fingerwag', 5),
    ...
    ('throw-object', 'throw-object', 5),
    ('throw-paper', 'throw-paper', 5))
if not base.config.GetBool('want-new-cogs', 1):
...
def loadTutorialSuit():
    loader.loadModelNode('phase_3.5/models/char/tt_a_ene_cgc_zero')
    loadDialog(1)
...
def loadSuitModelsAndAnims(level, flag=1):
    for key in ModelDict.keys():
        model, phase=ModelDict[key]
        if base.config.GetBool('want-new-cogs', 1):
...
def loadSkelDialog():
    ...
    SkelSuitDialogFiles=['Skel_COG_VO_grunt',
                         'Skel_COG_VO_murmur',
                         'Skel_COG_VO_statement',
                         'Skel_COG_VO_question']
    for file in SkelSuitDialogFiles:
        SkelSuitDialogArray.append(base.loader.loadSfx(loadPath + file + '.ogg'))

    SkelSuitDialogArray.append(SkelSuitDialogArray[0])
    SkelSuitDialogArray.append(SkelSuitDialogArray[1])
...
class Suit(Avatar.Avatar):
    ...
    def changeHandColor(self, r, g, b, alpha):
        cR=decode_sRGB_float(r)
        cG=decode_sRGB_float(g)
    ...
    def generateSuit(self):
        ...
        if dna.name == 'f':
            ...
            self.generateHead('yesman')
            self.setHeight(8.95)
        self.setName(SuitBattleGlobals.SuitAttributes[dna.dept][dna.name]['name'])
    ...
    def getLoseActor(self):
        ...
        if self.loseActor == None:
            if not self.isSkeleton:
                ...
                for key in ModelDict.keys():
                    models, phase=ModelDict[key]
                loseModel='phase_5/models/char/tt_a_ene_sc' + self.style.body + '_lose_zero'
                filePrefix, phase=TutorialModelDict[self.style.body]
                loseAnim='phase_' + str(phase) + filePrefix + 'lose'
                self.loseActor=Actor.Actor(loseModel, {'lose': loseAnim})
        ...
        if config.GetBool('want-smooth-frames', 1):
    ...
    def makeSkeleton(self):
        model='phase_5/models/char/tt_a_ene_sc' + self.style.body + '_zero'
